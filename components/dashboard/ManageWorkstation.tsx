...
export type ManageWorkstationProps = {
  events: ProgrammeEvent[];
};

export function ManageWorkstation({ events }: ManageWorkstationProps) {
    ...
    <>
      <header className="mb-lg">
        <div className="flex items-center justify-between gap-md mb-xs">
          <h1 className="text-[calc(28px*var(--text-scale))] font-bold tracking-[-0.02em] text-on-surface">Manage events</h1>
          <Link
            href="/events/new"
            className="inline-flex items-center gap-[6px] rounded-full bg-primary px-lg py-sm text-[13px] font-semibold text-on-primary hover:opacity-90 transition-opacity"
          >
            <span className="material-symbols-outlined text-[calc(18px*var(--text-scale))]" aria-hidden>add</span>New event
          </Link>
        </div>
        <p className="text-[calc(13px*var(--text-scale))] text-on-surface-variant">Filter, export, archive, or open an event to run it.</p>
      </header>
      ...
      {visible.length === 0 ? (
        ...
        <ul className="flex flex-col gap-sm">
          {visible.map((e) => <ManageEventCard key={e.id} e={e} selected={selected.has(e.id)} onToggle={() => toggle(e.id)} onDelete={() => deleteOne(e)} inBin={inBin} onRestore={() => execute('restore', [e.id])} pending={pending} />)}
        </ul>
      )}
...
function ManageEventCard({ e, selected, onToggle, onDelete, onRestore, inBin, pending }: {
      ...
      <div className="flex items-center justify-between sm:justify-end gap-md px-md pb-md sm:py-md sm:pr-md sm:pl-0 shrink-0">
        ...
        <div className="flex flex-col gap-xs w-[116px] shrink-0">
          {inBin ? (
            ...
            <>
              <Link
                href={`/events/${e.id}/details`}
                className={cn(buttonVariants({ variant: 'outline' }), 'w-full gap-xs px-sm py-sm font-label-md text-label-md')}
              >
                <span className="material-symbols-outlined text-[calc(16px*var(--text-scale))]" aria-hidden>info</span>Details
              </Link>
              <div className="flex gap-xs">
                <Link
                  href={`/events/${e.id}/edit`}
                  className={cn(buttonVariants({ variant: 'outline' }), 'flex-1 gap-xs px-sm py-sm font-label-md text-label-md')}
                >
                ...
                <Link
                  ...
                  className={cn(buttonVariants({ variant: 'outline' }), 'flex-1 gap-xs px-sm py-sm font-label-md text-label-md')}
                >
                  <span className="material-symbols-outlined text-[calc(16px*var(--text-scale))]" aria-hidden>how_to_reg</span>Check-in
                </Link>
              </div>
              <Link
                ...
                className={cn(buttonVariants({ variant: 'outline' }), 'w-full gap-xs px-sm py-sm font-label-md text-label-md')}
              >
                <span className="material-symbols-outlined text-[calc(16px*var(--text-scale))]" aria-hidden>bar_chart</span>Analytics
              </Link>
