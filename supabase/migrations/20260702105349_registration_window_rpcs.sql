-- Editor v4 (Design Session Log §"Flow rules locked 2026-06-20"):
-- registration window is an explicit open → close range. Organizer picks the
-- open date; an event can publish in advance and sit as Upcoming until the
-- open date arrives. Also plumbs the new `category` + window fields through
-- both event RPCs (the established pattern: adding an event field = re-create
-- the RPCs with the new columns).
