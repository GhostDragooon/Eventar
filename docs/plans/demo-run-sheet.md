...
1. Local Supabase stack up (`supabase start`) → `db reset` replays 63 migrations + seed (replay proven 2026-07-10; **app-against-local-stack proven end-to-end 2026-07-11**: landing → magic link via Mailpit → PKCE → staff dashboard, all green). Note: the committed seed's staff row is a placeholder literal (`MANAGER_EMAIL`) — unusable for login; `seed-demo.ts` owns creating the real demo operator (verified working: `demo-staff@local.test`).
2. Run `scripts/demo/seed-demo.ts` (built — see Milestone A): default org (no separate "Demo CPD Alliance" org — Task 2 shipped against the executable plan's simpler no-org-creation shape; nothing currently displays an org name to attendees, so this is cosmetic), event **"Clinical Update Seminar 2026"** (2 agenda blocks, 3 speakers, **pre-created as a draft** so Beat 2 demos the *publish* moment, not form-filling), 6 plausibly-named fake attendees, one pre-registered. **Fixture timing rule (corrected 2026-07-17 during Task 6 rehearsal — the original 45-min figure below was wrong, confirmed live):** event start = now + **90 min**, not 45. Registration closes UNCONDITIONALLY once the check-in window opens (`start − CHECKIN_OPEN_MINUTES` = 60 min — `lib/lifecycle/eventLifecycle.ts`, G11), regardless of `registration_close_at`. A 45-min offset put the event already past that boundary at the moment of seeding — the public page showed "Registration has closed" immediately, so Beat 3 could never succeed. 90 min leaves a genuine ~30-min registration-open window. Self-check-in (Beat 4) is unaffected by any of this timing either way — it's gated only by the static `events.checkin_modes.self_serve` flag the seed sets, not by the lifecycle clock — so it works regardless of how much real time has elapsed since seeding.
3. Demo server: **run from the demo worktree** — `bash /Users/ivan/Eventar-demo/scripts/demo/dev-local.sh` (or the `demo-local` launch config), port 3100. It must NOT run from the main checkout: Next 16 allows one dev server per project directory, and the always-on `:3000` server holds that lock. The script derives Supabase env from the running local stack. Next 16 dev already listens on the network interface (boot log prints the `Network:` URL) — for LAN phone scanning just set **`DEMO_SITE_URL=http://<laptop-LAN-IP>:3100`** before start so `lib/origin.ts` mints reachable QR URLs, and accept the macOS firewall prompt in prep, not live. Local-stack auth redirects for `:3100` (incl. a `192.168.*.*` wildcard) are already in `supabase/config.toml`.
