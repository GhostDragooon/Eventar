...
_Last updated: 2026-07-17 (**Milestone A — Tasks 2–10 + 6 done, code-complete but NOT yet phase-completion-protocol-passed**. Demo scripts (seed/reset/render-emails/ledger-demo) built and live-verified; full run-sheet driven end-to-end in a browser for the first time (Task 6), catching a real registration-timing bug; `set_staff_role()` + grant-hygiene migrations shipped to live Seoul (Tasks 9–10), each catching a real bug via RED→GREEN or fail-loud cleanup. **Still open: Task 12 (graph refresh, blocked on account session limit — 7/28 changed docs cached, 21 pending), Task 13 (three-lens phase-completion review, NOT run), Task 14 (formal close-out, NOT written).** **See `docs/plans/handoff_17072026.md` for the full session detail + exact resume line — do not skip Task 13.** Prior: handoff_12072026.md)_
