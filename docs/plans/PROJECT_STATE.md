...
> ## ⬆️ CURRENT — STAGE 8 IN PROGRESS (deploy & field-proven). Stage 7 shipped 2026-08-04.
>
...
> **Gates now:** tsc clean · eslint 0 errors (5 known `devEmailStub` warnings) · vitest **583 passed | 174 skipped** · `next build` clean, **22 routes** · migrations **92 local / 91 Seoul**.
>
> **⛔ SEOUL IS ONE MIGRATION BEHIND AND IT BREAKS THE LOOP.** `20260806000200_email_log_origin` is not applied there, and `lib/email/eventEmails.ts` selects *and* inserts `origin` unconditionally — so **every reminder and every survey fails with 42703 against the live database**. Loudly (rule 12 holds), but for every recipient. `handoff_08082026.md` called Seoul "2 behind (90 vs 92) — deliberate"; it is **1** behind, the other session's migrations already landed there, and the consequence was never assessed. Apply before anything points at Seoul.
>
> **⛔ `organisation_body_authorisations` has ZERO rows on Seoul** while 18 events carry an `accrediting_body_id`. DEFERRED 57 claims the migration "backfilled from reality … no in-flight accredited event was stranded" — on Seoul that is false, and `set_event_cpd_config()` therefore refuses every event. Cause unresolved: either the backfill no-op'd at apply time or the rows were removed after.
>
