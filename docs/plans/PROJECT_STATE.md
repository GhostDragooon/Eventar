...
_Last updated: 2026-07-25 (**CPD MVP attendance-verified issuance — Stages 0–8 ALL SHIPPED**. Config-free `award_attendance_credit()` wired into both check-in paths, live-browser-proven end-to-end (Karen Lau → real ledger row → chain valid); `reconcile-event.ts` for the recovery/retroactive-post path, also live-verified; and Stage 8's `freeze_cpd_config_if_credited()` BEFORE UPDATE trigger (Ivan confirmed include), live-verified against real Seoul data — blocks `cpd_hours`/`accrediting_body_id` changes once a credit references the event, allows everything else. Four schema mismatches and two live-only bugs (wrong Supabase project, kill-switch polarity) found and fixed earlier in the build — see the CPD MVP section below. **Still open: Ivan's carried inputs (price, first-meeting date, D0 flip, vault Sprint↔Milestone reconciliation) — nothing execution-side remains on this build.** Full detail: this doc's CPD MVP section below + `docs/plans/handoff_25072026.md`. Prior: Milestone A section below + `handoff_22072026.md`.)_
...
## CPD MVP — attendance-verified issuance: Stages 0–8 ALL SHIPPED (2026-07-25)
...
**Remaining:** nothing execution-side on this build · Ivan's outstanding inputs carried from `handoff_25072026.md`: `[PRICE]` in `docs/collateral/one-pager.md` + outreach email/phone, first internal-meeting/body-review date, D0 deploy flip (unrelated to this build, gates the *next* thing), whether to reconcile vault `CPD Roadmap — Backend First.md`'s Sprint body into the Milestone A–E spine · `credit_ledger` carries harmless dev-project residue on both Seoul (now 7 rows — one from this stage's test — and local) from live test/proof runs, accepted debt per `docs/DEFERRED.md` · `/Users/ivan/Eventar-demo` worktree needs `git checkout --detach main` before its next use (Stages 1–6 are now genuinely committed, no more manual file-copying needed) · `NewEventForm.test.tsx`'s pre-existing concurrency flake has shown up twice this week, always clean in isolation — promote to an active fix if it recurs a third time · next per `docs/plans/roadmap-to-mvp.md`: Milestone B or Milestone C, gated on Ivan's inputs above.
