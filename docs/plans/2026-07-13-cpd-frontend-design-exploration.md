...
**Detail-polish pass (Ivan's review of D):** three real defects found and fixed — (1) `.herozone{overflow:hidden}` was clipping the negative-margin laptop frame mid-card in every variant ("chopped, half-baked image"); removed, with `z-index` layering so the frame floats over the next section instead. (2) Landing CTAs were 9px-radius rectangles inside a fully-rounded pill nav — all landing buttons (nav, hero, CTA band) are now pill-shaped with normalised padding/line-height. (3) The `.sect:first-of-type` clearance selector was dead CSS in all variants (the herozone div is the first-of-type sibling), so the section under the hero never got its top padding — replaced with `.herozone+.sect`, verified via computed styles (94px applied, 30px clean gap under the bezel). The mini product window also gained a page-header row (name · body · cycle · On track) so it reads as a whole screen, not a fragment.

**On-page audience toggle + status-quo removed (2026-07-16, artifact label `audience-toggle-no-statusquo`):** Ivan — "get rid of the status quo… two landing pages, one for Practitioners, one for Organisers & Training Providers, add a toggle so each sees their dedicated page, clear labelling. DO NOT touch anything else." Done, landing only: (1) the whole "Two ways to…" comparison section (both variants) deleted — offer-only messaging now (hero → feature cards → CTA band). (2) The dev-chrome audience seg replaced by a real **on-page sliding segmented toggle** in the hero ("I'M A…" eyebrow + Practitioner / Organiser or training provider, each icon-labelled), uiverse-style (a `.athumb` sliding via `transform:translateX` off a `[data-on]` container attribute — pure CSS, no measurement, robust even while the landing is hidden). uiverse.io added to the toolbox (memory `uiverse-ui-elements-library`; WebFetch-blocked so patterns are inlined under the CSP). Both states verified (thumb slides, copy/URL swap, exactly one variant visible, zero console errors). Nothing else touched. *Note left for Ivan:* the hero product shot still shows the practitioner ledger in both variants — left as-is per "don't touch anything else"; swap to an organiser dashboard shot is a one-line follow-up if wanted.

**Audience messaging variants (2026-07-16, artifact label `audience-variants-copy`):** Ivan supplied two-audience landing copy (Practitioners / Organisers) and asked whether to drop the "problem today" framing. Call taken (agreed with Ivan, with pushback): no standalone problem section — but the contrast framing *inside* the copy stays ("Stop guessing / Stop losing / Stop dreading…" card leads are the strongest lines), and the problem statements became the "today" column of the comparison strip, where problem framing structurally belongs. Two copy corrections applied to Ivan's draft: **"digital passport" → "CPD Ledger"** (Q29 — competitor's brand), and B-card-1's "network of practitioners… demand finds you" softened to mechanism-framing ("list your event where practitioners look for accredited hours") since the network claim would be false pre-launch; "immutable" → "tamper-evident" (the provable claim). Built as an audience switcher (Practitioners default, `eventar.hk` ↔ `eventar.hk/organisers`), same layout + D aesthetic; hero H1/sub/chip/CTA, three feature cards, comparison strip and CTA band all swap. One systemic bug found by verification: the `hidden` attribute was defeated by explicit `display` values on `.herochip`/`.lgbtn`/`.featrow` (both variants rendered at once) — fixed globally with `[hidden]{display:none!important}`. All swaps assertion-verified.
...
## 4e. Mockup lapses closed — audit log, consent re-acceptance, credit query, page states (2026-08-01, artifact label `lapses-audit-consent-dispute-states`)

Step (3) of Ivan's 3→2→4 plan, continuing from `handoff_01082026.md` (which closed the Auth & onboarding lapse). The four remaining named lapses are now built. Everything was **grounded in the checked-in schema first**, not invented — each surface below cites the migration it reads.

| Lapse | Built | Schema it's grounded in |
|---|---|---|
| **Org audit-log viewer** | New 9th sidebar area (`/audit-log`). Chain-integrity banner + working **Verify chain** action (recompute → toast), event-type filter, 8-row table, row→drawer showing `chain_seq`, actor/subject ids, the `payload` JSON and both `prev_hash`/`hash` — with the links consistent between adjacent rows. | `20260704130400_init_audit_chain.sql` (`audit_events` columns, `verify_audit_chain`). Every `event_type` shown is a real literal a shipped definer function emits: `attendee_checked_in` (both the `self_check_in` and `staff_scan` variants, with their real payload shapes), `event_published`, `staff_role_changed`, `licence_verified`, `consent_granted`, `dsr_transitioned`, `session_revoked`. |
| **Consent re-acceptance** | Actionable banner on practitioner Overview + a rebuilt **Consent & legal** block in Settings (per-version rows, not two static cells) + a review modal naming what changed. Accepting flips the row to Current, clears the banner, toasts. | `20260704130500_init_consent_dsr.sql` (`consent_records` is versioned and append-only — a new version is a **new row**, never an edit) + `lib/legalVersions.ts` (`pp-0.2-draft` → `pp-0.3` is the real pin format). Copy states that accepting logs a `consent_granted` audit event and the prior acceptance stays on file. |
| **Credit query (dispute)** | New **Entry status** column on the Activity ledger. Per-row "Raise a query" → modal with a typed reason + free-text note; Submit is **disabled until both are present** (`reason` is NOT NULL). One row pre-seeded `Under review`. | `20260709270000_credit_ledger_entry_fn.sql` (`credit_disputes`: `ledger_entry_id`, `raised_by`, `reason`, the 5-value `status` CHECK, `resolution_reference`). The load-bearing copy — "a query never edits the entry; if upheld, a **new correcting entry** is appended that points back at this one" — is that migration's own comment, made visible. Status is deliberately **not** practitioner-movable: triage is staff work, matching the RLS (self read + self raise only). |
| **Empty / error / loading states** | A **Preview page state** switcher on the Events list swapping the real page between Data / Loading (skeleton) / Empty (first-run) / Error. Retry runs error → loading → data → toast. Plus a genuinely new System-sheet card: **error state — failed to load** (empty-state variants were already there; a failure state was not). | — (presentation) |

**Shared primitives added:** a real `showToast()` (the System sheet demoed toasts statically but nothing could fire one) and a working modal overlay (same — `.modaldemo` was a static picture).

**Three real defects found by verification, not by review:**
1. **Radio inputs inherited `.field input{width:100%}`** — the four reason options rendered as full-width boxes with their labels pushed outside. Found by screenshot, not by reading the CSS. Fixed with an explicit `.field .radiogrp input[type="radio"]` opt-out, commented so the next person doesn't "simplify" it away.
2. **`position:fixed` is wrong in this artifact.** The published frame reports `scrollHeight` to the host and grows to full content height, so a fixed overlay pins to a viewport far taller than the screen — the modal and toasts would have opened off-screen for a real reader. Caught by noticing that my two additions were the **only** `position:fixed` in the whole file: every prior overlay (`.scrim`, `.drawer`) is `position:absolute` inside `.shellwrap`. Converted both to absolute inside `.viewport`, added one toast host per app, and left a comment naming the reason. *This is convention-beats-novelty earning its keep — the existing pattern was right for a reason that isn't obvious locally.*
3. **A centred overlay in a tall app frame still opens below the fold** — added `revealOverlay()` (scroll the box into view on open). Verified the box lands fully on-screen (top 210, bottom 639, window 849).

**Verification:** 53 scripted assertions across two sweeps (every top-level tab → exactly one surface; all 9 organiser + 8 practitioner routes → a real page, none falling through to the stub; 7 event tabs; audit filter/drawer/verify; consent accept transition; dispute validation gating and submit; toast routing to the correct app's host; modal containment and fit), **0 failures, 0 console errors**, both themes sampled on every new component, and light restored on toggle-back. Navigation total is now **39 routes** (was 38 — the new Audit log area).

**Corrections to existing copy** (both were stale, and both would have read as lies if the stub ever showed): the organiser and practitioner "not designed yet" stubs still listed a subset of pages as "designed so far." All 9 + 8 are designed now and nothing routes to either stub; the copy says so and the stubs remain as the honest fallback for the next area added.

**Not built (flagged, not silently skipped):** dispute *resolution* (staff-side triage, the reviewer's queue) — it's a different actor's surface and the reviewer workflow is Sprint 3b's gated governance engine, so building it now would guess at a gated design. Same for the practitioner-side view of a resolved query once a correcting entry exists.
