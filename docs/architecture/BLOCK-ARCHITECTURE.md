...
|---|---|---|---|
| **B1 Events & Attendance** | ✅ shipped (pre-pivot backbone) | events, agenda_blocks, registrations, check-ins (self/staff), surveys, speaker check-ins, posters/QR, participation_evidence (ADR-0003 attendance-package evidence layer) | K1, K2, K3 |
| **B2 Professional Registry** | ✅ shipped (Stage 3); **Stage 10 in progress** — MCHK + 15 HKAM Colleges seeded 2026-08-13; **primary-licence-per-body shipped 2026-08-14** (Task 10.2/10.9 — `practitioner_licences_one_primary_idx` re-scoped to `(user_id, body_id)`, so one practitioner can hold `is_primary=true` at more than one body at once, per ADR-0001 §4(b); `declare_licence` gained `track`/`cycle_started_on` and a `status='active'` check on the body, closing DEFERRED item 30 — **block-contract tier change**, the function's signature and guard shape both widened); **the prior-approval deadline advisory shipped 2026-08-14** (Task 10.9/10.9 — `accrediting_bodies.cycle_config` carries a sourced `prior_approval` key per body, read-only, advises but never gates `set_event_cpd_config`); **multi-body-per-event newly in scope for 2026** (D-A, settled — `docs/adr/0002-multi-body-accreditation.md`): one event can carry many simultaneous accrediting bodies, reversing the "one event, one body" assumption `events.accrediting_body_id` (a single scalar UUID) was built on; **✅ schema + backfill + compatibility bridge shipped 2026-08-15** (Task 10.8/10.9, sub-task C3 — `20260815020000_accreditation_groups_and_roles.sql`): `event_accreditation_groups`/`event_accreditations`/`event_accreditation_occurrences`/`registration_roles`, the tie-validation guard, the credit-freeze extension, and `set_event_cpd_config()`'s existing single-body surface now transparently syncs into the new tables as a degenerate one-group case so no event falls through a gap with legacy columns set but no accreditation-group row; **✅ C5 (the multi-body organiser UI) shipped 2026-08-21** — `MultiBodyAccreditationWizard.tsx` + the granular CRUD RPCs (`add_event_accreditation_group` et al., `20260820000000` onward) let an organiser configure more than one group per event through the product, not just by hand; two dev-lens/user-lens review passes plus a same-day idempotency fix (duplicate-group guard on `add_event_accreditation_group`, `20260821020000`) closed before this note was written. **Not yet committed to git or pushed to Seoul as of this note** — schema is live on both local and Seoul (a prior session pushed it), the idempotency fix is local-only pending Ivan's go-ahead | accrediting_bodies, organisers, practitioner_licences + licence lifecycle functions, event_accreditation_groups/event_accreditations/event_accreditation_occurrences/registration_roles | K1, K2 |
| **B3 Credit Engine** | 🟡 core shipped (Stage 5, `credit_ledger`/`record_credit_entry`/`award_attendance_credit`) + **provisional per-body rule packs shipping in Stage 10, now needing to handle multiple bodies per event** (D-A); credit is already posted eagerly at check-in per Decision 10 — under multi-body that can mean up to ~13 provisional ledger rows per attendee per event, which stays safe only *if* a provisional-vs-confirmed attestation tier is reachable (**🟡 partially resolved 2026-08-15** — C4 still posts every body's row straight to `attestation_status='attendance_verified'` with no provisional/confirmed distinction at posting time, but `confirm_credit_entry()` (`20260815050000_credit_ledger_body_confirmation.sql`) now makes the `body_confirmed` tier reachable after the fact — a `body_admin`/`eventar_staff` actor can post a new `credit_confirmed` row referencing the original, append-only; **as of 2026-08-16 (Q39)**, confirming a row that already has a `credit_adjusted`/`credit_revoked` compensating entry against it is never blocked, but is flagged via a new `audit_events` row (`20260816060000_confirm_credit_entry_compensating_flag.sql`, `event_type='credit_confirmation_flagged'`) so the coexistence is queryable; no UI/queue exists yet to surface which rows need confirming OR which confirmed rows carry a flag — see R5/M4, `docs/adr/0002-multi-body-accreditation.md`, `docs/doctrine.md`'s OPEN forks and `docs/DEFERRED.md`); the *versioned* `body_rules`/deterministic-evaluator half stays 🔒 gated → **Stage 13** (body review + Q26, doctrine D.1); **✅ the storage shape the award engine reads is shipped 2026-08-15** (Task 10.8/10.9, sub-task C3, shared with B2 above) — `event_accreditation_groups`/`event_accreditations`/`event_accreditation_occurrences` exist, are backfilled from every pre-existing accredited event, and the tie-validation trigger enforces ADR-0002's "Award selection" rule at write time (**corrected same-day by C4** — the original guard compared cardinality alone and rejected ADR-0002's own worked example, HK College of Pathologists' Day1/Day2/Both shape; it now compares actual occurrence sets — see the C4 migration, `20260815030000_multi_body_award_engine.sql`); **✅ C4's award-selection engine shipped 2026-08-15** (same migration) — `award_attendance_credit()` now iterates every accreditation group on the event and posts zero/one/several `credit_ledger` rows, one per body that both accredits the event and where the practitioner holds a verified licence, resolving role→category per ADR-0001/R1 and applying each group's own `proportional`/`explicit_schedule` scheme; **✅ C5 (the multi-body organiser UI) shipped 2026-08-21** — see the B2 row above for detail; a real multi-body config now exists through the product, not only by hand | versioned body_rules/cycles, the deterministic evaluator (bounded grammar — never a general rules engine), `credit_ledger` + `record_credit_entry`, disputes, issuance wiring, roster ingestion | B1 attendance, B2 registry, K2 mechanics |
...
| **B9 Rule Authoring / Source Evidence** | 📋 admitted 2026-08-29 (Q41), post-MVP — slots alongside or after Q26 / doctrine D.1 work, not on the current grant-MVP week's critical path | Python FastAPI + worker on a separate container host (Next.js stays on Vercel). Owns: documents, document_versions, document_relationships, document_approvals, processing_jobs, parse_runs, parsed_elements, chunks, embedding_indexes + per-index embedding tables, retrieval_index_assignments, pack_versions, pack_sources, pack_source_passages, evaluation_cases, retrieval_traces. Definer functions for every audited mutation; RLS for all reads. Never writes to B3 tables (guideline §1.2, §3.5 item 15). Governing spec: `rag_building_guideline_v0.5.x_eventar.txt`. Admission checklist: [B9-admission-2026-08-29.md](B9-admission-2026-08-29.md) | K1 (reads only), K2 (audit_events mechanics), K3 (withSecurity at the Next.js edge that fronts /retrieve) |
...
Turnstile/strike ladder → K3 · staff MFA-mandatory → K1 (S4) · pg_cron emails + Resend cutover → B5 · certificate-issued notification → B5 (client: B4) · evidence packs → B6 · roster ingestion → B3 · VCs/TSA anchor → B4 bolt-ons · cross-jurisdiction polymorphism → B2/B3 accepted debt · `set_staff_role()` audit → K1 (mechanics from K2) · pricing/billing → **new block B10 (Commercial)** when it becomes code at all (invoice-first = zero code, no block needed yet; re-tagged from B9 → B10 on 2026-08-29 when B9 was admitted for RAG) · multi-body accreditation → B2/B3 · prior-approval deadline advisory → B2 (reads accrediting_bodies data, no new block) · rule-pack authoring / source-evidence retrieval → **new block B9 (Rule Authoring / Source Evidence)** admitted 2026-08-29, see `docs/architecture/B9-admission-2026-08-29.md` and Decisions Log Q41; governed by `rag_building_guideline_v0.5.x_eventar.txt`.
